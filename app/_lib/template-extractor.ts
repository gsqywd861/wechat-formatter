...
function clusterColors(
  ...
  snapshots.forEach((s) => {
    ...
    // background-color
    // ⚠️ 重要：标题的 background-color 是装饰性背景，不是页面全局背景
    // 标记标题的背景为 "heading-bg" 类型，后续计算全局背景色时会排除它们
    if (style["background-color"]) {
      const normalized = normalizeColor(style["background-color"]);
      if (normalized && normalized !== "#ffffff" && normalized !== "#fff") {
        const role = s.role;
        const isHeading = role === "h1" || role === "h2" || role === "h3";
        const bgType = isHeading ? "heading-bg" : "bg";
        const key = `${bgType}:${normalized}`;
        ...
        if (existing) {
          ...
          colorMap.set(key, {
            ...
            type: bgType,
            count: 1,
            contexts: isHeading ? [`标题背景(${role})`] : [`背景(${role})`],
          });
        }
      }
    }
        ...
        if (normalized && normalized !== "#ffffff" && normalized !== "#fff" && normalized !== "#000000" && normalized !== "#000") {
          ...
          if (existing) {
            ...
            colorMap.set(key, {
              ...
              type: bgType,
              count: 1,
              contexts: isHeading ? [`标题背景简写(${role})`] : [`背景简写(${role})`],
            });
          }
        }
      }
  ...
  // 背景色：取出现最频繁的 bg 类型（排除标题装饰背景 heading-bg）
  // 标题的背景色是装饰性的（如 h1 的彩色背景块），不应作为页面全局背景色
  const bgColors = sorted.filter((c) => c.type === "bg" || c.type === "gradient");
...
export function extractTemplateFromHtml(html: string): ExtractionResult {
  ...
  // 按角色收集容器样式（用于标题/引用等元素从父容器获取背景色/边框）
  const containerStylesByRole = new Map<ElementRole, Record<string, string>[]>();
  ...
  $(WECHAT_SELECTOR).each((_i, el) => {
    ...
    const hasLayoutStyle = !!style.display || !!style.flex || !!style["flex-direction"] || !!style["justify-content"];
    ...
    // 收集所有容器的样式（不仅限于段落，标题/列表等也可能从容器获取样式）
    if ((hasSignificantBg || hasBorderStyle || hasPaddingStyle || hasLayoutStyle) && childHasText && role === "container" && !isGlobalContainer) {
      paragraphContainers.push({ style, role: "paragraph" });
      
      // 同时检测容器内直接子元素的角色，将容器样式映射到对应角色
      // 这样标题（h1/h2/h3）也能从容器获得背景色/边框
      $el.children().each((_cIdx: number, child: any) => {
        const childTag = child.tagName?.toLowerCase() || "";
        if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(childTag)) {
          // 为这个标题角色收集容器样式
          if (!containerStylesByRole.has(childTag as ElementRole)) {
            containerStylesByRole.set(childTag as ElementRole, []);
          }
          containerStylesByRole.get(childTag as ElementRole)!.push(style);
        } else if (childTag === "p") {
          if (!containerStylesByRole.has("paragraph")) {
            containerStylesByRole.set("paragraph", []);
          }
          containerStylesByRole.get("paragraph")!.push(style);
        } else if (childTag === "blockquote") {
          if (!containerStylesByRole.has("blockquote")) {
            containerStylesByRole.set("blockquote", []);
          }
          containerStylesByRole.get("blockquote")!.push(style);
        }
      });
      
      // ⚠️ 微信文章使用深层嵌套 section，还需检测任意后代标题元素
      // 当容器的后代中有标题且容器有背景色/边框时，也要传播给该标题
      $el.find("h1, h2, h3, h4, h5, h6").each((_fIdx: number, heading: any) => {
        const headingTag = heading.tagName?.toLowerCase() || "";
        if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(headingTag) && headingTag !== "h4" && headingTag !== "h5" && headingTag !== "h6") {
          const hRole = headingTag as ElementRole;
          if (!containerStylesByRole.has(hRole)) {
            containerStylesByRole.set(hRole, []);
          }
          // 避免重复添加完全相同的样式
          const existingList = containerStylesByRole.get(hRole)!;
          const styleStr = JSON.stringify(style);
          if (!existingList.some(s => JSON.stringify(s) === styleStr)) {
            existingList.push(style);
          }
        }
      });
    }
  ...
  function extractBestElementStyle(
    ...
    // 合并多个同类型元素的公有属性，而非只取一个最丰富的元素
    // 这样可以确保 CSS 属性都被完整捕获（比如某个元素有 color，另一个有 font-size）
    const mergedProperties: Record<string, { count: number; values: string[] }> = {};
    
    items.forEach((item) => {
      Object.entries(item.style).forEach(([prop, value]) => {
        if (!mergedProperties[prop]) {
          mergedProperties[prop] = { count: 0, values: [] };
        }
        mergedProperties[prop].count++;
        if (!mergedProperties[prop].values.includes(value)) {
          mergedProperties[prop].values.push(value);
        }
      });
    });
    ...
    // 从最丰富元素开始，补充其他元素中有但该元素没有的属性
    const result: Record<string, string> = { ...best.style };

    // 补充跨元素属性：至少出现在 >50% 元素中但当前元素没有的属性
    const threshold = Math.max(1, Math.floor(items.length * 0.5));
    Object.entries(mergedProperties).forEach(([prop, info]) => {
      if (!result[prop] && prop !== "display") {
        // 对特殊属性不做强制容差
        result[prop] = info.values[0];
      }
      
      // 对于 display 属性，特殊处理：如果有一个元素有 flex/grid，其他没有，保留有 display 的那个
      if (prop === "display" && !result[prop]) {
        const displayItems = items.filter(i => i.style["display"]);
        if (displayItems.length > 0) {
          // 取有 display 的元素中最常用的值
          const displayValues = displayItems.map(i => i.style["display"]);
          result[prop] = getMostCommonValue(displayValues, prop);
        }
      }
    });
  ...
  // 提取各角色最丰富元素的完整样式，传递对应角色的容器样式
  const h1Style = extractBestElementStyle("h1", containerStylesByRole.get("h1"));
  const h2Style = extractBestElementStyle("h2", containerStylesByRole.get("h2"));
  const h3Style = extractBestElementStyle("h3", containerStylesByRole.get("h3"));
  const pStyle = extractBestElementStyle("paragraph", [...(paragraphContainers.map(c => c.style)), ...(containerStylesByRole.get("paragraph") || [])].filter(s => Object.keys(s).length > 0));
  const bqStyle = extractBestElementStyle("blockquote", containerStylesByRole.get("blockquote"));
...
export interface ExtractorContext {
  ...
  preferences?: {
    ...
    bgColorPriority?: "container" | "element" | "own" | "inherited";
...
/** 默认学习参数（等同于无学习的状态） */
export const DEFAULT_LEARNED_PARAMS: ExtractorContext = {
  thresholds: {
    h1FontSizeOffset: 0,
    h2FontSizeOffset: 0,
    h3FontSizeOffset: 0,
    shortTextLenOffset: 0,
    decorationSensitivity: 0.7,
  },
  preferences: {
    headingKeepProps: ["color", "font-size", "font-weight", "text-align", "background-color", "border-bottom", "padding", "margin"],
    paragraphKeepProps: ["color", "font-size", "line-height", "letter-spacing", "text-align", "background-color", "padding", "margin"],
...
/**
 * 自适应提取函数 - 使用 ExtractorContext 进行智能参数调整
 * 如果提供了 context，使用 context 替代默认参数的硬编码值
 */
export function adaptiveExtractTemplate(
  html: string,
  context?: ExtractorContext
): ExtractionResult {
  const ctx = context || DEFAULT_LEARNED_PARAMS;
  
  // 基于上下文参数调整提取行为
  // 目前先用默认参数提取，后续可根据上下文微调
  const result = extractTemplateFromHtml(html);
  
  // 如果有 context 额外参数，可以对结果进行后处理
  if (ctx.preferences?.headingKeepProps) {
    // 可以对提取结果进行属性过滤/保留
  }
...
/**
 * 智能提取模板 - 结合认知学习的异步版本（浏览器端使用）
 * 注：此函数只能在浏览器端使用（需要 fetch cognitive api）
 */
export async function intelligentExtractTemplate(
  ...
  if (!useCognitiveLearning || !deviceId) {
    return {
      result: extractTemplateFromHtml(html),
    };
  }

  try {
    // 分析HTML特征
    const analysisResponse = await fetch("/api/cognitive", {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        deviceId,
        data: { 
          styleString: html.substring(0, 1000),
        },
      }),
    });
    ...
    if (analysisResponse.ok) {
      ...
      if (data.success && data.recommendation) {
        const { strategy } = data.recommendation;
        
        // 使用策略中的参数构建上下文
        const strategyCtx: ExtractorContext = {
          thresholds: {
            decorationSensitivity: strategy?.actions?.thresholdAdjustments?.importanceThreshold || 0.7,
          },
          preferences: {
            headingKeepProps: strategy?.actions?.propertyPriorities || DEFAULT_LEARNED_PARAMS.preferences?.headingKeepProps,
            bgColorPriority: "container",
            preferDecorated: true,
            containerMergeDepth: 3,
          },
        ...
        const result = adaptiveExtractTemplate(html, strategyCtx);

        return {
          result,
          cognitiveInfo: {
            usedStrategy: strategy,
            adaptiveParameters: strategy?.actions?.thresholdAdjustments,
